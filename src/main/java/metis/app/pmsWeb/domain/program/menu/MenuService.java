...
public class MenuService extends BaseService<Menu, Long> {
    ...
    public void deleteMenu(List<Menu> menus) {
        if (ArrayUtils.isNotEmpty(menus)) {
            delete(menus);
            menuMapper.deleteAuthGroups(menus);
            menus.stream().filter(menu -> isNotEmpty(menu.getChildren())).forEach(menu -> {
                deleteMenu(menu.getChildren());
                for(int i = 0; i < menu.getChildren().size(); i ++ ){
                    deleteAuthGroup(menu.getChildren().get(i).getMenuId());
                }
            });
        }
    }
    ...
    public void deleteAuthGroup(Long menuId){
        menuMapper.deleteAuthGroup(menuId);
    }
}
