...
public class AXBootAuthenticationFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if(((HttpServletRequest) req).getRequestURI().contains(".jsp")) {

                // 화면 이동시 로그인 정보가 있으면 SYSTEM_LOGIN_USER 테이블에 기존 로그인 사용자 세션 정보 UPDATE.
                if(SessionUtils.getSessionUser() != null) {
                    tokenAuthenticationService.updateLoginUser(SessionUtils.getSessionUser().getUserCd());
                }
            }
