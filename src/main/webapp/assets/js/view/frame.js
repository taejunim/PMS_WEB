...
//PCS 상태 이력 index 찾기
function findPcsStatusHistoryMenu(element){
    let pcsProgramCode  = ['operation-schedule', 'history-pcs'];
    if(pcsProgramCode.includes(element.progCd)) return true;
}
//CPCM 상태 이력 index 찾기
function findCpcmStatusHistoryMenu(element){
    let cpcmProgramCode = ['operation-contract', 'history-ac-converter', 'history-dc-converter'];
    if(cpcmProgramCode.includes(element.progCd)) return true;
}
//일정 등록, 수정, 삭제 시 메인 화면에서 보이는 충방전량
function reloadSchedule(){
    $($('#content-frame-container').get(0)).children().get(0).contentWindow.reloadSchedule();
}
...
fnObj.pageStart = function () {
    ...
    this.menuItems[0].open = this.menuItems.length > 0 ? true : false;
...
fnObj.topMenuView = axboot.viewExtend({
    initView: function () {
        ...
        var menuItems = fnObj.menuItems;
        //고정형 이동형 여부에 따라 CPCM이력 / PCS 이력 숨김

        menuItems.forEach(function (n) {
            if(isPositionFixYn === "true") {                //고정형
                if(n.children.findIndex(findCpcmStatusHistoryMenu) > -1)
                    n.children.splice(n.children.findIndex(findCpcmStatusHistoryMenu), 1);
                n.children.forEach(function (child) {
                    if(child.children.findIndex(findCpcmStatusHistoryMenu) > -1)
                        child.children.splice(child.children.findIndex(findCpcmStatusHistoryMenu), 1);
                    if(child.children.findIndex(findCpcmStatusHistoryMenu) > -1)
                        child.children.splice(child.children.findIndex(findCpcmStatusHistoryMenu), 1);
                });
            } else if(isPositionFixYn === "false") {        //이동형
                if(n.children.findIndex(findPcsStatusHistoryMenu) > -1)
                    n.children.splice(n.children.findIndex(findPcsStatusHistoryMenu), 1);
                n.children.forEach(function (child) {
                    if(child.children.findIndex(findPcsStatusHistoryMenu) > -1)
                    child.children.splice(child.children.findIndex(findPcsStatusHistoryMenu), 1);
                });
            } else {                                        //등록된거 없음
                if(n.children.findIndex(findCpcmStatusHistoryMenu) > -1)
                    n.children.splice(n.children.findIndex(findCpcmStatusHistoryMenu), 1);
...
fnObj.tabView = axboot.viewExtend({
    ...
    print: function () {
        ...
        } else if(isPositionFixYn === "false") {
            this.list.splice(0,1);
        //등록된 ESS 없을때 일단 고정형 표출
        } else {
            this.list.splice(1,1);
        }
    ...
    open: function (item) {
        ...
        if (findedIndex < 0) {
            ...
            /*fnObj.tabView.list.forEach(function (_item, idx) {
                if (idx != 0 && _item.menuId != item.menuId) {
                    fnObj.tabView.close(_item.menuId);
                }
            });*/
        }
