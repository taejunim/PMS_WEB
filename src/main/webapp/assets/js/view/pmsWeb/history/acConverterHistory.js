...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    ITEM_CLICK: function (caller, act, data) {
        axboot.ajax({
            method: "GET",
            url: ["acConverter", "selectConverterDetail"],
...
fnObj.searchView = axboot.viewExtend(axboot.searchView, {
    initView: function () {
        ...
        this.operationModeFilter = $("#operationModeFilter");
        //날짜 picker UI
    ...
    getData: function () {
        return {
            ...
            operationModeStatus: this.operationModeFilter.val()
        }
    }
});
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                ...
                {key: "operationStatusName", label: "운전 상태", width: 110, align: "center"},
                {key: "operationModeStatusName", label: "운전 모드", width: 110, align: "center"},
                {key: "totalPower", label: "총 계통 전력 (kw)", width: 120, align: "center" ,formatter: function(){
                        return (this.value == null ? '-' : ax5.util.number(this.value,{"money": true}));}},
                {key: "totalActiveCurrent", label: "총 계통 유효 전류 (A)", width: 150, align: "center", formatter: function(){
                        return (this.value == null ? '-' : ax5.util.number(this.value,{"money": true}));}},
                {key: "totalVoltage", label: "총 계통 전압 (V)" , width: 120, align: "center", formatter: function(){
                        return (this.value == null ? '-' : ax5.util.number(this.value,{"money": true}));}},
                {key: "internalTemp", label: "함체 내부 온도 (℃)", width: 120, align: "center", formatter: function(){
                        return (this.value == null ? '-' : ax5.util.number(this.value,{"money": true}));}},
                {key: "transformerTemp", label: "교류 변압기 온도 (℃)", width: 150, align: "center", formatter: function(){
                        return (this.value == null ? '-' : ax5.util.number(this.value,{"money": true}));}},
                {
                    key: "errorFlag",
                    label: "오류 발생",
...
fnObj.formView01 = axboot.viewExtend(axboot.formView, {
    ...
    setData: function (data) {
        ...
        for (var i in data) {
            ...
            $(set + " .totalCurrent").text(ax5.util.number(data[i].totalCurrent,{"money": true}));
            $(set + " .outputVoltage").text(ax5.util.number(data[i].outputVoltage,{"money": true}));
            $(set + " .outputFrequency").text(ax5.util.number(data[i].outputFrequency,{"money": true}));
            $(set + " .gridVoltage").text(ax5.util.number(data[i].gridVoltage,{"money": true}));
            $(set + " .gridFrequency").text(ax5.util.number(data[i].gridFrequency,{"money": true}));
            $(set + " .gridPhaseDifference").text(ax5.util.number(data[i].gridPhaseDifference,{"money": true}));
            $(set + " .powerFactor").text(ax5.util.number(data[i].powerFactor,{"money": true}));
            $(set + " .acCurrent").text(ax5.util.number(data[i].acCurrent,{"money": true}));
            $(set + " .dcVoltage").text(ax5.util.number(data[i].dcVoltage,{"money": true}));
            $(set + " .dcOffset").text(ax5.util.number(data[i].dcOffset,{"money": true}));
            $(set + " .activeCurrent").text(ax5.util.number(data[i].activeCurrent,{"money": true}));
            $(set + " .activeCurrentContrast").text(ax5.util.number(data[i].activeCurrentContrast,{"money": true}));
            $(set + " .reactiveCurrentContrast").text(ax5.util.number(data[i].reactiveCurrentContrast,{"money": true}));
            $(set + " .stackTemp").text(ax5.util.number(data[i].stackTemp,{"money": true}));
            $(set + " .inductor1Temp").text(ax5.util.number(data[i].inductor1Temp,{"money": true}));
            $(set + " .inductor2Temp").text(ax5.util.number(data[i].inductor2Temp,{"money": true}));
