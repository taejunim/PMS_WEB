...
var ACTIONS = axboot.actionExtend(fnObj, {
    PAGE_SEARCH: function (caller, act, data) {
        var searchDate = caller.searchView.getData().searchDate;
        if(searchDate.length != 7) return axToast.push("검색 일자를 설정하여 주십시오.");
        axboot.ajax({
            type: "GET",
            url: ["/chargeDischargeStatistic", "monthlyList"],
    ...
    EXCEL_EXPORT: function (caller, act, data) {
        var fileName = "충방전월간통계_"+ date + ".xls";
...
fnObj.gridView01 = axboot.viewExtend(axboot.gridView, {
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                {key: "label", label: "통계 일", width: 160, align: "center"},
                {key: "accumulatedCharge", label: "누적 충전 전력량 (kWh)", width: 150, align: "center"},
                {key: "accumulatedDischarge", label: "누적 방전 전력량 (kWh)", width: 150, align: "center"},
            ],
            //그리드 summary
            footSum: [
                [
