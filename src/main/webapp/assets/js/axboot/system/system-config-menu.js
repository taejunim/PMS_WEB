...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    PAGE_SAVE: function (caller, act, data) {
        ...
        axboot
            ...
            .done(function () {
                if(formData.menuId !== '' && formData.progCd === '') {
                    axboot.ajax({
                        type: "GET",
                        url: ["menu", "auth", "delete"],
                        data: {"menuId": formData.menuId},
                        callback: function (res) {
                            ACTIONS.dispatch(ACTIONS.PAGE_SEARCH);
                        }
                    });
                } else if (formData.progCd) {
                    axboot.ajax({
                        type: "PUT",
                        url: ["menu", "auth"],
    ...
    SELECT_PROG: function (caller, act, data) {
        ...
        caller.treeView01.updateNode(_data);
        setNoMessageDiv2("noDataMessage01", 1);
        if(_data.progCd) {
            axboot.ajax({
                type: "GET",
                url: ["menu", "programAuth"],
                data: {"progCd": program},
                callback: function (res) {
                    var list = [];
                    caller.formView01.authGroup.forEach(function (g) {
                        ...
                        if (res) {
                            $.extend(item, {
                                program_schAh: res.schAh || "N",
                                program_savAh: res.savAh || "N",
                                program_exlAh: res.exlAh || "N",
                                program_delAh: res.delAh || "N",
                                program_fn1Ah: res.fn1Ah || "N",
                                program_fn2Ah: res.fn2Ah || "N",
                                program_fn3Ah: res.fn3Ah || "N",
                                program_fn4Ah: res.fn4Ah || "N",
                                program_fn5Ah: res.fn5Ah || "N"
                            });
                        }
                        list.push(item);
                    });
            ...
            callback: function (res) {
                ...
                if (res.program) {
                    ...
                    if(list.length > 0) {
                        ...
                        let defaultAuthGroup = caller.formView01.authGroup;
                        defaultAuthGroup.forEach(function (n) {
                            $.extend(n, {
                                useYn: "N",
                                schAh: "N",
                                savAh: "N",
                        ...
                        caller.gridView01.setData(defaultAuthGroup);
                    }
                } else setNoMessageDiv2("noDataMessage01", 0, "권한 그룹 설정을 위하여 프로그램을 선택하여 주십시오.");
            }
        });
    },
...
fnObj.pageStart = function () {
    ...
    axboot
        .call({
            type: "GET", url: "/programs", data: "",
            callback: function (res) {
                var programList = [];
                res.map.list.forEach(function (n) {
                    programList.push({
        ...
        .call({
            type: "GET", url: "/commonCodes", data: {groupCd: "AUTH_GROUP", useYn: "Y"},
            callback: function (res) {
                var authGroup = [];
                res.list.forEach(function (n) {
                    authGroup.push({
        ...
        axboot.buttonClick(this, "data-page-btn", {
            ...
            "save": function () {
                axDialog.confirm({
                    ...
                    if (this.key === "ok") ACTIONS.dispatch(ACTIONS.PAGE_SAVE);
                });
            }
        });
    }
                ...
                if (this.value[0]) {
                    ...
                    axDialog.confirm({
                        ...
                        if (this.key === "ok") {
                            ...
                            $('[data-ax5combobox="progCd"]').ax5combobox("setValue", program);
                        }
                    });
                }
            }
